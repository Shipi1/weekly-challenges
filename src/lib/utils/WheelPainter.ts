...
export default class WheelPainter {
  ...
  drawWheelNoCache(context: Context, wheel: Wheel) {
    // Draw slices directly (no intermediate cache)
    context.save();
    context.translate(context.canvas.width / 2, context.canvas.height / 2);
    const radius = getWheelRadius(context);
    ...
    // Draw center directly (no intermediate cache)
    if (wheel.config.type === "color") {
      ...
      context.arc(
        0,
        0,
        getWheelRadius(context) * hubSizes[wheel.config.hubSize],
        0,
        2 * Math.PI,
      );
...
const getWheelRadius = (context: Context) =>
  (Math.min(context.canvas.width, context.canvas.height) / 2) * 0.85;

const createInMemoryImage = (context: Context) =>
  new OffscreenCanvas(context.canvas.width, context.canvas.height).getContext(
    "2d",
  )!;
...
const readBlobAsDataUri = (
  ...
  new Promise<string>((resolve) => {
    ...
    reader.onloadend = (event) =>
      resolve(onLoad ? onLoad(event) : (event.target!.result as string));
